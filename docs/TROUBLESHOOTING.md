...
| `✳ rate-limited, retrying` | HTTP 429 from the endpoint, with no cached reading to fall back on. There is no growing backoff — the next poll tries again, and this clears once the endpoint does. When there *is* a cached reading you see those numbers instead of this message. |
